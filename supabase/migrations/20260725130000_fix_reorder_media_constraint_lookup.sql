-- The media order constraint is deferrable and therefore validates the
-- multi-row reorder at the end of the UPDATE statement. Avoid looking up the
-- constraint by name while the function uses an intentionally empty search
-- path.
